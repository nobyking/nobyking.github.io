...
var tips = {
    ...
    8 : function(){
        html = "<div class='popContTit'>亲爱的用户</div>\
                ...
                <div class='P_btns'>\
                    <a href='http://dldir1.qq.com/invc/tt/QQBrowser_Setup_Privilegecenter.exe#?ADTAG=SNO=' class='btn btn_downloadQB' onclick='report(\"BROWSER.ACTIVITY.YEWEN.DOWNLOADQBCLICK\")'>下载QQ浏览器</a>\
                </div>";
        return html;
    },
    7 : function(k){
        ...
        html = "<div class='popContTit'>亲爱的用户</div>\
                ...
                <div class='P_btns'>\
                    <a href='http://dldir1.qq.com/invc/tt/QQBrowser_Setup_Privilegecenter.exe#?ADTAG=SNO=' class='btn btn_downloadQB2' onclick='report(\"BROWSER.ACTIVITY.YEWEN.DOWNLOADQBCLICK2\")'>下载专用版QQ浏览器</a>\
                </div>";
        return html;
    },
    6 : function(){
        ...
        if(k == 1){
            html = "<div class='popContTit'>QQ浏览器</div>\
                    <div class='contentGifts'>\
                        <h3 class='gifts_title'>\
                            <p>恭喜您</p>\
                            <p class='tit'>获得《叶问3》电影票一张</p>\
                        </h3>\
                        <div class='giftsCdkey'>兑换码为：" + cdkey + "</div>\
                        <div class='giftsExplain' style=''>\
                            <div class='giftsTips' style=''>\
                                <p class='ptit'>使用规则：</p>\
                                <p class='rule_order'><span>1)</span><em>兑换券有效期：自兑换之日起有效期一个月，请尽早使用</em></p>\
                                <p class='rule_order'><span>2)</span><em>仅支持【抠电影】在线选座使用；所支持影院，请查询网站<a class=\"weblink\" href=\"http://www.komovie.cn?ADTAG=SNO=\" target=\"_blank\">www.komovie.cn</a>或抠电影App；</em></p>\
                                <p class='rule_order'><span>3)</span><em>仅限售价85元以下的场次使用，2D3D通兑每张订单可用一张</em></p>\
                                <p class='ptit'>使用方法：</p>\
                                <p><span></span><em>登录【抠电影】—>选择电影《叶问3》场次、座位—>兑换码支付—>成功购票；</em></p>\
                                <p><span></span><em>抠电影客服热线：400-000-9666</em></p>\
                            </div>\
                        </div>\
                    </div>";
        } else if(k == 2){
            html = "<div class='popContTit'>QQ浏览器</div>\
                      <div class='contentGifts'>\
                        <h3 class='gifts_title'>\
                            <p>恭喜您</p>\
                            <p class='tit'>获得《叶问3》电影红包一个 </p>\
                        </h3>\
                        <div class='giftsCdkey'>兑换码为：" + cdkey + "</div>\
                        <div class='giftsExplain'>\
                            <div class='giftsTips' style=''>\
                                <p class='ptit'>使用规则：</p>\
                                <p class='rule_order'><span>1、</span><em>注册并登录抠电影(<a class=\"weblink\" href=\"http://www.komovie.cn?ADTAG=SNO=\" target=\"_blank\">www.komovie.cn</a>)->选择所在城市->选择在线选座影院->选择场次座位->支付时选择使用抵值券+现金支付>订票成功后将收到含取票码短信，依据提示到影院换票观影。</em></p>\
                                <p class='rule_order'><span>2、</span><em>15元代金券限制35元以上场次可用，每张订单可用一张。如，9.9元的特惠是不能使用的。</em></p>\
                                <p class='rule_order'><span>3、</span><em>有效期至2016年5月31日</em></p>\
                            </div>\
                        </div>\
                    </div>";
        }
...
(function (window,$) {
    Page = qv.zero.extend(qv.zero.AbstractPage, {
        ...
        getLucky: function () {
            ...
            zUtil.ensureLogin(function () {
                ...
                report("BROWSER.ACTIVITY.YEWEN.GETLUCKY");
            });
        },
        //查询中奖记录
        queryGift: function () {
            zUtil.ensureLogin(function () {
                zHttp.send({actid: 89341}, function (res) {
                    if (res.ret == 0) {
                        var _htmlArr = [],
                            _html = '',
                            count = 0,
                        ...
                        for (var i = res.data.op.length - 1; i >= 0;i--) {
                            var obj = res.data.op[i].val,
                                ...
                                if (name === "叶问3电影票") {
                                    ...
                                    _htmlArr.push('<li><span class="giftsTime">' + time + '</span><span class="giftsInfo">' + name + '</span><span style="cursor: pointer; text-decoration: underline;" class="cdkey" ONCLICK="dialog.open(tips[\'0\'](2, \''+ cdkVal +'\'))">兑换码:'+cdkVal+'</span></li>');
                                };
                            count++;
                        }
                        if (count == 0) {
                ...
                report("BROWSER.ACTIVITY.YEWEN.QUERYGIFTSCLICK");
            });
        }
    });
    ...
    var shareFunc = function(type,url){
        report('BROWSER.ACTIVITY.YEWEN.SINA');
        var _url = "http://tq.qq.com/events/yewen/index.html?ADTAG=sina";
        var _pic = "http://event.browser.qq.com/stdl/activity/yewen/img/share.jpg";
        var _title = "#QQ浏览器邀你看首映，免费电影票等你领！";
        var _description = "《叶问3》重磅归来，QQ浏览器请你去电影院看甄子丹张晋激战咏春、甄子丹泰森上演世纪大战，荷尔蒙霸屏，燃爆三月，抢票请戳";
