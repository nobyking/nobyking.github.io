    ...
    (function (window,$) {
        Page = qv.zero.extend(qv.zero.AbstractPage,{
            ...
            addMoney: function(key, cb){
                ...
                if (key){
                    zUtil.ensureLogin(function(){
                        zHttp.send({'actid' : 51422}, function (res) {
                            if (res.ret == 0) {
                                setMoney('++');
                                isWatching = true;
                            }
                    ...
                    window.api.clickReport('BROWSER.ACTIVITY.NZCM.SEEVIDEOCLICK');
                }
            },
            getLucky: function(){
                if(!window.api.isQQBrowser()){
                ...
                zUtil.ensureLogin(function(){
                    zUtil.ensureLogin(function(){
                        ...
                        zHttp.send({actid: 51423, _record_def_gift: 1}, function (res) {
                            if (res.ret == 0) {
                ...
                window.api.clickReport('BROWSER.ACTIVITY.NZCM.GETLUCKYCLICK');
            },
            queryGift : function(){
                zUtil.ensureLogin(function() {
                ...
                window.api.clickReport('BROWSER.ACTIVITY.NZCM.QUERYGIFTCLICK');
            },
            queryMoney: function(){
                zHttp.send({'actid': 51434}, function (res) {
                    if(res.ret == 0){
            ...
            queryIsAddMoney : function(){
                zHttp.send({ actid : 51430},function(res){
                    if(res.ret == 0) {
                        isSetQuestion = res.data.op.join['51421'];
                        isWatching = res.data.op.join['51422'];
                    }
                });
            }
        });
